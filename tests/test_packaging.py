...
class PackagingTests(unittest.TestCase):
    def test_versions_match(self):
        self.assertEqual("0.2.14", docker_tray.APP_VERSION)
        self.assertIn("pkgver=0.2.14", (ROOT / "packaging/arch/PKGBUILD").read_text())
        self.assertIn('version="${1:-0.2.14}"', (ROOT / "package-deb.sh").read_text())
    ...
    def test_release_version_script_accepts_current_tag(self):
        ...
        result = subprocess.run(
            [ROOT / "scripts/release-version", "v0.2.14"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            check=True,
        )
        self.assertEqual("0.2.14", result.stdout.strip())

    def test_release_version_script_rejects_mismatched_tag(self):
        import subprocess

        result = subprocess.run(
            [ROOT / "scripts/release-version", "v0.2.15"],
            cwd=ROOT,
            capture_output=True,
            text=True,
        )
